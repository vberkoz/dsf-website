...
export const Menu = () => {
  ...
  return (
    <div style={{ zIndex: 2001 }} className="sticky top-0 flex justify-between bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 border-b-2 border-gray-200 dark:border-gray-800">
      ...
      <div className="flex">
        ...
        <Link className="py-2 px-3 hover:bg-gray-200 dark:hover:bg-gray-800" href="/">
          ...
          <div className="float-left flex">
            {menuItems.map((item, key) => (
              <div key={key} className="flex flex-col">
                {item?.subitems ? (
                  <div className="group">
                    <Link href={`/${item.href}`} className={`${ item.href === currentPageset[0]?.pageset && "bg-gray-200 dark:bg-gray-700" } flex cursor-pointer p-3 hover:bg-gray-200 dark:hover:bg-gray-800`}>
                      {item.title}
                    ...
                    <div className="absolute z-10 hidden flex-col bg-gray-100 dark:bg-gray-900 shadow group-hover:flex">
                      {item.subitems.map((subItem, subItemKey) => (
                        <Link key={subItemKey} href={`/${subItem.href}`} className={`${ subItem.href === currentPage && "bg-gray-200 dark:bg-gray-700" } flex cursor-pointer p-3 hover:bg-gray-200 dark:hover:bg-gray-800`}>{subItem.title}</Link>
                      ))}
                    </div>
                  </div>
                ) : (
                  <Link href={`/${item.href}`} className={`${ item.href === currentPageset[0]?.pageset && "bg-gray-200 dark:bg-gray-700" } flex cursor-pointer p-3 hover:bg-gray-200 dark:hover:bg-gray-800`}>{item.title}</Link>
                )}
              </div>
            ))}
          </div>
